"""Email channel for the PM Intelligence Agent.

Builds the professional HTML intelligence email and the daily digest email,
and sends them via SMTP. The rendering/sending logic is unchanged from the
original monolithic Notifier — it was only relocated into this module.
"""
...
from src.notifications._common import _CATEGORY_META, _score_color
...
class EmailRenderer:
    """Builds email subjects/bodies and sends the report + digest emails."""

    @staticmethod
    def build_subject(alert_trends: list[Trend], date_str: str) -> str:
        if alert_trends:
    ...
    @staticmethod
    def make_email_safe(html: str) -> str:
        """
    ...
    @staticmethod
    def build_fallback_html(alert_trends: list[Trend], date_str: str) -> str:
...
<table width="100%" cellpadding="0" cellspacing="0" dir="ltr" style="background:#eef2f7;padding:28px 0 40px;">
  <tr>
    ...
    def send_email(
        ...
        subject = EmailRenderer.build_subject(alert_trends, date_str)

        email_body = EmailRenderer.build_professional_email(
    ...
    @staticmethod
    def send_digest_email(digest_articles, stats, alert_trends, report_path) -> None:
