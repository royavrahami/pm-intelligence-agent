...
All failures are caught and logged – the agent never crashes due to notification errors.
...
# PM-specific category icons for notifications
_CATEGORY_ICONS = {
    "project_management": "📋",
    "program_management": "🗂️",
    "agile":              "🔄",
    "leadership":         "🎯",
    "strategy":           "🧭",
    "ai_pm":              "🤖",
...
class Notifier:
    ...
    def _send_email(
        ...
        """Build and send a rich HTML email with the PM intelligence report."""
    ...
    def _build_subject(alert_trends: list[Trend], date_str: str) -> str:
        if alert_trends:
            return f"🚨 [{date_str}] PM Intelligence – {len(alert_trends)} trends require attention"
    ...
    def _build_email_html(
        ...
        """
        Build a clean, email-client-compatible HTML body for PM intelligence reports.
                  ...
                  <tr>
                    <td style="padding:14px 18px;">
                      ...
                      <p style="margin:4px 0 0;color:#166534;font-size:12px;opacity:0.75;">
                        All monitored PM trends are within normal momentum thresholds
                      </p>
                    </td>
                  </tr>
                </table>
  ...
  <tr>
    <td align="center" style="padding:0 12px;">
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <!-- ── Header ─────────────────────────────────────────────── -->
        <tr>
          <td style="background:linear-gradient(135deg,#0f3460 0%,#1e40af 100%);padding:28px 32px;">
              ...
              <tr>
                <td align="center">
                  <span style="background:{'#fef2f2' if alert_trends else '#f0fdf4'};
                    ...
                    {'🚨 ' + str(len(alert_trends)) + ' active alert' + ('s' if len(alert_trends) != 1 else '') if alert_trends else '✅ All clear — no active PM alerts'}
                  </span>
                </td>
              </tr>
            </table>
              ...
              <tr>
                <td style="padding:14px 18px;">
                  <p style="margin:0;font-size:13px;color:#1d4ed8;line-height:1.6;">
                    ...
                    Open the HTML file in your browser for the complete PM trend landscape,
        ...
        <tr>
          <td style="background:#f8fafc;border-top:1px solid #e5e7eb;
            ...
            <p style="margin:0;color:#9ca3af;font-size:11px;">
              PM Intelligence Agent &nbsp;·&nbsp; Auto-generated &nbsp;·&nbsp; {date_str}
            </p>
          </td>
        </tr>
    ...
    def _send_digest_email(self, digest_articles, stats, alert_trends, report_path) -> None:
        """Send the full PM digest as an HTML email with report attached."""
        now = datetime.now(timezone.utc)
        date_str = now.strftime("%d %b %Y")
        ...
        kw_str = " ".join(
            f'<span style="background:#dbeafe;color:#1e40af;border-radius:9px;padding:2px 8px;margin:2px;font-size:0.8em;">{kw}</span>'
...
<body style="font-family:system-ui,sans-serif;background:#f5f7fa;margin:0;padding:0;">
  ...
  <div style="max-width:960px;margin:0 auto;padding:24px 20px;">
    {alert_block}
    <div style="margin:20px 0;"><strong style="color:#0f3460;">🔤 Top PM Keywords:</strong><br><br>{kw_str}</div>
    ...
    <p style="margin-top:16px;color:#9ca3af;font-size:0.8em;">📎 Full interactive report attached as HTML file.</p>
  </div>
