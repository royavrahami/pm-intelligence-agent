"""
GitHub Collector – PM Intelligence Agent.
Scrapes GitHub Trending and uses the GitHub REST API to surface
repositories relevant to project/program management in high-tech.
...
_USER_AGENT = "PMIntelligenceAgent/1.0"
_REQUEST_TIMEOUT = 15

# GitHub topics most relevant to PM/PgM in high-tech
_GITHUB_TOPICS = [
    "project-management",
    "program-management",
    "agile",
...
class GitHubCollector:
    """
    Collects trending and topic-filtered GitHub repositories relevant to PM/PgM.

    Args:
        source_repo:  Repository for Source records.
        article_repo: Repository for Article records.
        github_token: Optional personal access token (increases rate limit from 60 to 5000/hr).
    """
    ...
    def collect_all(self, sources: list[Source]) -> int:
        ...
        logger.info("GitHub Collector: %d new PM-related items total", total)
        return total

    # ── Trending Page Scraper ──────────────────────────────────────────────────

    def _collect_trending_page(self, sources: list[Source]) -> int:
        """Scrape github.com/trending for repositories trending in PM/tooling space."""
    ...
    def _collect_topic_search(self, sources: list[Source]) -> int:
        """Use GitHub search API to find recently updated repos for PM-relevant topics."""
        api_source = self._source_repo.upsert(
            name="GitHub API – PM Topic Search",
    ...
    def _search_topic(self, source: Source, topic: str) -> int:
        """Query the GitHub search API for a single PM-related topic."""
        params = {
        ...
        for repo in data.get("items", []):
            ...
            content = (
                f"GitHub Repository (PM Topic: {topic})\n"
            ...
            # Map PM-related topics to appropriate categories
            category_map = {
                "agile": "agile",
                "scrum": "agile",
                "kanban": "agile",
            ...
            category = category_map.get(topic, "project_management")
