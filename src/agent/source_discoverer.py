"""
Source Discoverer – PM Intelligence Agent self-expansion module.

The agent analyses existing articles to discover links to NEW PM/PgM sources
it hasn't monitored before, then records them in the database.

Discovery strategies:
  1. Link extraction: scan article URLs for known RSS patterns
  2. LLM-guided discovery: ask the LLM for PM-relevant source recommendations
"""
...
_LLM_DISCOVERY_PROMPT = """You are a research librarian specialising in Project Management, Program Management,
and Engineering Leadership in high-tech companies.

Based on the following list of sources and topics I'm already monitoring, suggest 5 NEW,
high-quality sources I should add to stay up-to-date on PM/PgM practices.

Focus on:
- Newsletters, blogs, or podcasts with RSS/Atom feeds about PM/PgM
- Agile methodology resources (Scrum, Kanban, SAFe, LeSS)
- Engineering leadership and management publications
- OKR, roadmapping, and strategic planning resources
- AI tools for project/program management

For each suggestion, return a JSON object:
{
  "name": "<Source name>",
  "url": "<Full URL of the RSS feed or main page>",
  "source_type": "<rss | web | arxiv>",
  "category": "<project_management | program_management | agile | leadership | strategy | ai_pm | tools>",
...
class SourceDiscoverer:
    """
    Autonomously discovers new PM/PgM information sources.
    ...
    Args:
        ...
        model:          OpenAI model (optional override).
    """
    ...
    def discover(self) -> int:
        """
        Run all discovery strategies and register new PM/PgM sources.
        ...
        logger.info("SourceDiscoverer: discovered %d new PM sources", new_sources)
        return new_sources
        ...
        for article in articles[:20]:
            ...
            if feed_url:
                registered = self._register_new_source(
                    ...
                    discovery_method="rss_mining",
                    reason=f"Discovered via article: {(article.title or '')[:60]}",
                )
                if registered:
                    new_count += 1
    ...
    def _find_rss_on_page(self, article_url: str, domain: str) -> Optional[str]:
        """Look for an RSS feed link on an article's page or try common paths."""
        try:
    ...
    def _discover_via_llm(self) -> int:
        """Ask the LLM to recommend new PM/PgM sources based on current monitoring."""
    ...
    def _register_new_source(
        ...
        logger.info("New PM source discovered: %s (%s)", name, url)
        return True
