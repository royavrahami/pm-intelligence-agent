"""
Daily Digest Agent – PM Intelligence Agent.
End-of-day summary agent.

Runs once per day and produces a comprehensive daily digest of everything
collected during the day, framed for Project/Program Managers.
...
Digest content per article:
  ...
  - Relevance score
  - Category (PM, Agile, Leadership, Strategy, etc.)

Report structure:
  1. Day-in-numbers banner (totals, categories breakdown)
  2. Alert trends
  3. Full sortable table of all articles
  4. Category-grouped article cards (with PM-focused summaries)
...
class DigestArticle:
    """
    Lightweight, serialisable view of an article for the PM digest report.
    ...
    keywords: list[str]
    published_date: str     # e.g. "02 Mar 2026 14:30 UTC"
    collected_date: str     # e.g. "02 Mar 2026 17:21 UTC"
...
@dataclass
class DigestStats:
    """Aggregate statistics for the daily PM digest banner."""
    date_str: str
...
class DailyDigestAgent:
    """
    End-of-day PM intelligence digest agent.

    Collects all articles gathered today, enriches them with keywords,
    and generates a comprehensive PM-focused digest report.

    Args:
        lookback_hours: How many hours back to consider as "today" (default 24).
    ...
    def run(self) -> Optional[Path]:
        """
        Execute the PM daily digest cycle.

        Returns:
            Path to the generated HTML digest report, or None if no data.
        """
        logger.info("=" * 65)
        logger.info("PM Daily Digest Agent: starting at %s", datetime.now(timezone.utc).isoformat())
        logger.info("=" * 65)
        ...
        with get_session() as session:
            ...
            if not raw_articles:
                logger.info(
                    "PM Daily Digest: no articles found for the past %d hours",
                    self._lookback_hours,
                )
                return None

            logger.info("PM Daily Digest: processing %d articles", len(raw_articles))
        ...
        if report_path:
            logger.info("PM daily digest report: %s", report_path)
            notifier = Notifier()
    ...
    def _build_digest_articles(self, articles: list[Article]) -> list[DigestArticle]:
        """Convert ORM Article objects into DigestArticle view objects with PM keywords."""
    ...
    def _build_stats(self, articles: list[DigestArticle]) -> DigestStats:
        """Compute aggregate statistics for the PM digest header."""
        if not articles:
