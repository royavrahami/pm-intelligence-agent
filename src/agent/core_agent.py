"""
Core Agent Orchestrator – PM Intelligence Agent.
...
class CoreAgent:
    """
    Autonomous PM Intelligence Agent.
    ...
    def run(self) -> Optional[Path]:
        """
        Execute a full PM intelligence cycle.

        Returns:
            Path to the generated HTML report, or None on failure.
        """
        logger.info("=" * 70)
        logger.info("PM Intelligence Agent: starting run at %s", datetime.now(timezone.utc).isoformat())
        logger.info("=" * 70)
        ...
        with get_session() as session:
            ...
            try:
                ...
                logger.info("Loaded %d active PM sources", len(active_sources))

                # ── Step 1: Collect ──────────────────────────────────────────
                collected = self._collect(active_sources, source_repo, article_repo)
                ...
                total_trends_in_db = trend_repo.get_top_trends(limit=100, days=30)
                logger.info(
                    "Trends: %d new this run | %d total in DB",
                    len(trends), len(total_trends_in_db),
                )
                session.flush()

                # ── Step 4: Discover New Sources ──────────────────────────────
                new_sources = self._discover_sources(source_repo, article_repo, expansion_repo)
                run.new_sources_discovered = new_sources
                session.flush()

                # ── Step 5: Generate Report ───────────────────────────────────
                report_path, report_articles = self._generate_report(
                    article_repo, trend_repo, run.id,
                ...
                logger.info(
                    "PM Agent run complete: collected=%d, processed=%d, trends=%d, new_sources=%d",
                ...
                logger.exception("PM Agent run failed: %s", exc)
                raise
    ...
    def _analyse_trends(
        ...
        """Run PM-focused trend detection if API key is available."""
        if not settings.openai_api_key:
            logger.info("Skipping trend analysis – no OPENAI_API_KEY configured")
            return []
        analyzer = TrendAnalyzer(article_repo=article_repo, trend_repo=trend_repo)
        return analyzer.analyse(lookback_days=30)  # Full 30-day window, independent of current run
    ...
    def _discover_sources(
        ...
        """Discover and register new PM/PgM information sources."""
    ...
    def _generate_report(
        ...
        REQ-07: filters articles seen in previous reports before capping.
        ...
        # REQ-07: filter out URLs already shown in previous reports
        if seen_repo:
        ...
        # Trends: last 30 days from the full DB – always independent of dedup
        trends = trend_repo.get_top_trends(limit=10, days=30)

        if not articles and not trends:
            logger.info("No PM report generated – nothing to report")
            return None, []
        ...
        # REQ-07: mark the displayed articles as seen so they won't re-appear
    ...
    def _load_sources_config() -> dict:
        ...
        try:
            ...
            logger.error("Failed to load PM sources config: %s", exc)
            return {}
