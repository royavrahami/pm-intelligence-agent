"""
Content Processor – PM Intelligence Agent.
The pipeline stage that transforms raw articles into enriched, scored,
AI-summarised records ready for PM-focused reporting.
...
class ContentProcessor:
    ...
    Args:
        ...
        summarizer:    Configured Summarizer instance (optional; skip if no API key).
    ...
    def process_pending(self) -> tuple[int, int]:
        ...
        if not articles:
            logger.info("ContentProcessor: no pending articles to process")
            return 0, 0
            ...
            try:
                ...
                if score >= self._min_score and self._summarizer:
                    ...
                    if result:
                        ...
                        article.pm_relevance = result.get("pm_relevance")
                        total_summarised += 1
