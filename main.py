...
def cmd_run(_args: argparse.Namespace) -> None:
    """Execute one full PM intelligence cycle and exit."""
    from src.agent.core_agent import CoreAgent

    console.print("[bold blue]Starting single PM Intelligence Agent run...[/bold blue]")
    agent = CoreAgent()
...
def cmd_schedule(args: argparse.Namespace) -> None:
    """Start the recurring PM intelligence scheduler."""
...
def cmd_report(_args: argparse.Namespace) -> None:
    """Generate a PM report from data already in the database."""
    ...
    with get_session() as session:
        ...
        if not articles and not trends:
            console.print(
                "[yellow]No data available. Run the agent first with: python main.py run[/yellow]"
            )
            return
        ...
        console.print(f"[bold green]PM Report generated:[/bold green] {path}")
...
def cmd_status(_args: argparse.Namespace) -> None:
    ...
    if not runs:
        console.print("[yellow]No runs found. The PM agent hasn't run yet.[/yellow]")
        return

    table = Table(
        title="Recent PM Agent Runs",
        box=box.ROUNDED,
...
def cmd_digest(args: argparse.Namespace) -> None:
    """Run the end-of-day PM Daily Digest Agent."""
    from src.agent.daily_digest_agent import DailyDigestAgent

    hours = getattr(args, "hours", 24)
    min_score = getattr(args, "min_score", 30)
    console.print(
        f"[bold blue]Running PM Daily Digest Agent (last {hours}h, min score {min_score})...[/bold blue]"
    )
    ...
    if report_path:
        console.print(f"\n[bold green]✓ PM daily digest saved:[/bold green]\n  {report_path}")
    else:
...
def cmd_sources(_args: argparse.Namespace) -> None:
    ...
    table = Table(
        title=f"Registered PM Sources ({len(sources)} active)",
        box=box.ROUNDED,
        header_style="bold white on #0f3460",
    )
    ...
    for name, source_type, category, fetch_count, relevance_boost in sources:
        table.add_row(name[:50], source_type, category, str(fetch_count), str(relevance_boost))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pm-intelligence-agent",
        description=(
            "Autonomous PM Intelligence Agent – stay ahead of the curve in "
            "Project & Program Management, Agile, and Engineering Leadership."
        ),
    ...
    digest = subparsers.add_parser("digest", help="Run end-of-day PM digest and send by email")
